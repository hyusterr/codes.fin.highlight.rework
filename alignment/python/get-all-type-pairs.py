...
@click.option('--sbert_neg_threshold', '-sn', type=float, default=0.9011)
@click.option('--lda_neg_threshold', '-ln', type=float, default=0)
...
def main(directory, corpus, rouge_sim_threshold, sbert_neg_threshold, lda_neg_threshold, print_type1):
    ...
    for filename in tqdm(os.listdir(directory)):
        ...
        for line in data:
            ...
            if rouge <= rouge_sim_threshold: # 0.0784
                if print_type1:
                ...
                print(f'TYPE_2\t{last_year_ID}\t{this_year_ID}\t{corpus_dict[last_year_ID]}\t{corpus_dict[this_year_ID]}') # \t{rouge}\t{sbert}')
            else:
                if print_type1:
                    print(f'TYPE_0\t{last_year_ID}\t{this_year_ID}\t{corpus_dict[last_year_ID]}\t{corpus_dict[this_year_ID]}')
